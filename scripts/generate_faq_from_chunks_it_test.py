...
INPUT_FILE = Path("data/chunks/it_support_chunks.jsonl")
OUTPUT_FILE = Path("data/processed/by_category/it_support.jsonl")
...
8. "keywords" should contain 3 to 6 lowercase English keywords.
9. "verified" must always be false.
10. Generate at most {MAX_FAQ_PER_CHUNK} FAQ entries from this chunk.
11. Return only a valid JSON array. Do not use markdown. Do not explain.
...
def main():
    ...
    print(f"Processing {len(selected_chunks)} chunks from {INPUT_FILE}")
    ...
    for index, chunk in enumerate(selected_chunks, start=1):
        ...
        try:
            ...
            valid_items = [item for item in faq_items if is_valid_item(item)]
            all_faqs.extend(valid_items)

            print(f"  Generated {len(valid_items)} valid FAQ entries")
